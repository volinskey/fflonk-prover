...
impl Keccak256Transcript {
    ...
    pub fn add_g1_point(&mut self, point: &G1Affine) {
        self.buf.extend_from_slice(&fq_to_canonical_be_bytes(&point.x));
        self.buf.extend_from_slice(&fq_to_canonical_be_bytes(&point.y));
    }
...
mod tests {
    ...
    fn reproduces_multiplier_alpha_from_reference_chain() {
        ...
        t.add_scalar(&fr("8656540919485434325127996463862963077422400187418464280627379020094348872551")); // ql
        t.add_scalar(&fr("0")); // qr
        t.add_scalar(&fr("2614769233458373575914167661386876499189977403679308856418227786252928171678")); // qm
        t.add_scalar(&fr("19273473638380901646332238083870398589358386996736725487279976400322880323939")); // qo
        t.add_scalar(&fr("0")); // qc
        t.add_scalar(&fr("2149601555970763550502742290511066346210126521388222512348169637526747700236")); // s1
        t.add_scalar(&fr("7665469566589217530047082740024900379575949551084655260788477394062900215820")); // s2
        t.add_scalar(&fr("9593052032151367161561845615248167687278938119063653268474063026781260594803")); // s3
        t.add_scalar(&fr("16127061633303507620994494118418859935014078291353908877586231175433014908086")); // a
        t.add_scalar(&fr("10481924070205961981688681564524817045099149903371547605480211094869994165600")); // b
        t.add_scalar(&fr("11165319260432401010175261312139144201227551818404445502013201559202664335104")); // c
        t.add_scalar(&fr("414154269577338230123695308905609275000771520577836775585168828854428911021")); // z
        t.add_scalar(&fr("20032753347252750623757752495127622395615307912815516003156251467213315967927")); // zw
        t.add_scalar(&fr("528472514911415871465957436906207785247458062589561186936125913093451716131")); // t1w
        t.add_scalar(&fr("1735584312925944748061585464799496108892046647899363999711364062499048168279")); // t2w
