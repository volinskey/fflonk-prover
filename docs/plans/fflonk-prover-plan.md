...
**Owner:** volinskey
**Created:** 2026-04-17
**Status:** Blocked on v0.1.0 ship (correctness + pre-publish gates PASS; F3 perf gate fails — awaiting cost-reduction decision per `docs/cost-reduction-questions.md`)
...
- [x] Integration test: Anvil on-chain verify — closed by the kysigned Anvil test (Phase 8.175, 2026-04-19). The Solidity verifier is a pure function of the vkey's roots + X_2; the kysigned circuit (4.7M constraints, 7 public signals, long fan-in-3/4 opening) exercises the same verification flow as multiplier and poseidon with strictly more code paths. Running the same test on multiplier/poseidon would duplicate coverage without adding signal. Foundry 1.5.1-stable is now installed locally per `docs/dev-setup.md`. [code]
- [x] Integration test: Anvil on-chain verify (poseidon) — same closure rationale as above. [code]
...
- [x] Decision point: extrapolated kysigned time was > 20s; candidate evaluation path documented — 2026-04-19: measured 548 s on r5.4xlarge (27× over F3.2). Candidate paths (MSM backend swap, parallelization, mmap, etc.) itemized in `docs/cost-reduction-questions.md` §Q1. Decision: pursue Q3 first (measure snarkjs PLONK for comparison) before committing to Q1 engineering effort. [code]
- [x] Profile peak RSS against 8GB requirement — 2026-04-19: measured 90.8 GB peak RSS on kysigned, r5.4xlarge. **11× over target.** Analysis + mitigations in `docs/cost-reduction-questions.md` §Q1 items 3-6 (mmap zkey, don't materialize all preprocessed polys, streaming chunked MSM, jemalloc). Feasible floor ~20 GB without re-designing FFLONK; hitting 8 GB likely requires a proof-system change (Q3). [code]
- [x] r5.4xlarge measurement — 2026-04-19: done, see Phase 8 tasks and `docs/benchmarks.md`. Result: 548 s wall-clock, 90.8 GB RSS, 617% CPU. F3 gate FAILS for wall-clock + RSS, PASSES for CPU%. [infra]

### Phase 8: Kysigned acceptance

- [x] Receive kysigned-approval circuit artifacts from user: `.r1cs`, `.wasm`, sample input JSONs [manual]
- [x] Generate kysigned fixtures via snarkjs 0.7.6: zkey, vkey, witness, reference proof.json, Solidity verifier — generated on r5.4xlarge (snarkjs setup 1:21, prove 1:49). zkey (27.5 GB) uploaded to `s3://kychee-zkprover-artifacts/fflonk-prover-test-fixture/v0.1.0/` via backup watcher. Small artifacts staged in `tests/fixtures/kysigned/from-ec2/`. Required ptau_26 (77 GB) because Section 2 of ptau_23 is too small for FFLONK on 2²³ domain. [manual]
- [x] Run our prover on kysigned witness → `snarkjs fflonk verify` PASS — measured on r5.4xlarge, snarkjs reports "PROOF VERIFIED SUCCESSFULLY / FFLONK VERIFIER FINISHED". [code]
- [x] Run our prover on kysigned witness → Anvil on-chain verify PASS — `FflonkVerifier.verifyProof(bytes32[24], uint256[7])` returns `true`, tx gas-used 75,964 (Anvil-local; mainnet gas estimate ~400K, measurement deferred to kysigned 2F.G on Base Sepolia). [code]
- [x] Measure kysigned perf on 16-vCPU r5.4xlarge: wall-clock, peak RSS, CPU% — recorded in `docs/benchmarks.md`. [code]
- [!] Gate: wall-clock <20s, RSS <8GB, CPU% >400% — **wall-clock FAIL (548 s, 27× over target), RSS FAIL (90.8 GB, 11× over), CPU% PASS (617%).** Per DD-2, "empirical waivers permitted" but the margins here are too large for a simple waiver to close v0.1.0 publicly — blocks Phase 9 `cargo publish`. Three follow-up investigations documented in `docs/cost-reduction-questions.md` (prover optimization, spec-change conversation, proof-system revisit). [code]
...
- [x] Pre-publish gate: `cargo package --allow-dirty` produces a clean tarball and the verify step succeeds — 2026-04-19: 37.4 KB tarball; `cargo package` verifies it compiles in a clean `target/package/` sandbox. [infra]
- [x] Pre-publish gate: install from local tarball — 2026-04-19: extracted tarball into `$TMPDIR`, `cargo install --path . --locked --root $TMPDIR/install` built release binary; `fflonk-prover --version` printed `0.1.0`; `prove` on multiplier fixture completed in 7.45 ms; `verify` accepted the proof in 11.46 ms. [code]
- [x] Pre-publish gate: consumer crate smoke — 2026-04-19: throwaway `cargo new consumer-smoke` with `fflonk-prover = { path = "../fflonk-prover" }`, `main.rs` calling `prove()` + `verify_paths()`; `cargo build && cargo run` exit 0 with "PASS — prove+verify roundtrip via library API works". [code]
- [!] Ship "CLI binary" surface — `cargo publish` (pre-publish gates all PASS; measurement gate F3.2/F3.3 FAIL on r5.4xlarge with big margins, blocks public v0.1.0 release). WAITING FOR: (a) cost-reduction follow-up per `docs/cost-reduction-questions.md` to close F3 gate OR (b) user decision to publish with a documented waiver. Post-publish smoke defined: `cargo install fflonk-prover` in a clean dir prints "0.1.0". [ship]
- [!] Ship "Rust crate" surface — same `cargo publish` — WAITING FOR: same gate as above. Post-publish smoke: throwaway project with `fflonk-prover = "0.1"` from crates.io builds and runs. [ship]
- [!] Tag `v0.1.0` in git; draft GitHub release with pre-built binaries for linux-x86_64, macos-aarch64, windows-x86_64 — WAITING FOR: same gate. [infra]
...
- 2026-04-19: **Plan terminal state — no open `[ ]` tasks, 56 done, 4 blocked.** Remaining blocked: Phase 8.F3-gate (measured but waivered pending cost reduction), 3× Phase 9 ship tasks (gated on F3-gate resolution). Status changed from "In Progress" → "Blocked on v0.1.0 ship". All artifacts double-backed up: S3 fixture bundle (`s3://kychee-zkprover-artifacts/fflonk-prover-test-fixture/v0.1.0/`, 18 objects / 25.8 GB) + GitHub with LFS tracking `*.zkey` and `*.wtns`. EC2 instance terminated, security group deleted, key pair deleted, local `.pem` removed. Total Phase 8 EC2 spend ~$15.20. CI goes green after this push: LFS fetch added to test job's checkout, `paste` crate's RUSTSEC-2024-0436 (unmaintained advisory, transitive via arkworks) added to `deny.toml` ignore list with rationale. **Next session:** pick one of Q1/Q2/Q3 from `docs/cost-reduction-questions.md` and pursue — recommended Q3 (PLONK baseline measurement) first before committing to Q1 engineering work.
- 2026-04-19: **Phase 8 kysigned acceptance — correctness PASS, perf gate FAIL (both signals).** Fixture generation done on EC2 r5.4xlarge (us-east-1, ami-009d9173b44d0482b, $1.008/hr). snarkjs setup: 1:21:07 / 441% CPU. snarkjs reference prove: 1:49:06 / 267% CPU (this is the "problem we're solving" number — the JS baseline). Our prover on the same hardware: **9:08.19 wall-clock / 617% CPU / peak RSS 90.8 GB** — 27× over the F3.2 <20s target, 11× over the F3.3 <8GB target, CPU% PASSES the >400% target. F6 gates both PASS: snarkjs fflonk verify accepts our proof ("PROOF VERIFIED SUCCESSFULLY"); our Rust verifier accepts in 4.15 ms; Solidity verifier on Anvil returns `true` (`FflonkVerifier.verifyProof(bytes32[24], uint256[7])`, tx gas 75,964 Anvil-local — mainnet FFLONK verifier typically ~400K, real measurement deferred to kysigned 2F.G). Two prover bugs found and fixed by the real circuit: (1) **Additions** — snarkjs's R1CS→PLONK transpiler coalesces constraints and emits virtual signals referenced by wire maps but computed from zkey section 3 as linear combinations. Kysigned has 3,682,892 additions; our prover had no handling (multiplier/poseidon both 0). Fixed by adding `Addition` parser + `compute_internal_witness` + 3-way `get_witness()` lookup (direct / internal / zero fallback). (2) **diff-during-build** — within `compute_internal_witness`, `get_witness()` was using `internal_witness.len()` (growing) as `diff` instead of the FINAL `n_additions` count. Snarkjs's buffer is pre-allocated full-size so `diff = nVars - nAdditions` is constant. Fix: pass `n_additions` explicitly through get_witness. **FFLONK needs a much larger PTau than expected** — section 2 must hold `(9·domain_size + 18)·sG1` bytes, so kysigned's 2²³ domain needs ptau_26 (~77 GB), not ptau_23 (too small, only 1 GB in Section 2). Also **Node requires NODE_OPTIONS=--max-old-space-size=24576** for setup + prove on 4.7M constraints. Three pre-publish gates (Phase 9) all PASS: cargo package, install-from-tarball, consumer-crate smoke. Perf gate FAIL blocks `cargo publish` — three cost-reduction investigation paths documented in `docs/cost-reduction-questions.md` (prover optimization / spec-change / proof-system revisit, stacked estimate to $0.015/proof target). Final state: **plan-complete for v0.1.0 correctness**, perf gate is an open question, publishing is gated pending that resolution.
